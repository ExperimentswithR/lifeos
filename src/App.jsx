...
function AppInner({ auth }) {
  const [data, setData, dataLoaded] = auth?.user ? useSupabaseData(auth.user, D0) : [null, null, true];
  const [localData, setLocalData] = useState(D0);
  const actualData = data || localData;
  const setActualData = data !== null ? setData : setLocalData;
  ...
  const pg = {
    ...
    admin: isAdmin && supabase ? <Admin supabase={supabase} toast={push}/> : null
  };
  ...
  return (
    <div style={{background:"#f4f2f0",color:txt,minHeight:"100vh",fontFamily:"'Outfit','Nunito',sans-serif",fontSize:14,lineHeight:1.6}}>
      <style>{anim}</style>
      <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet"/>
